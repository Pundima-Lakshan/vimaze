...
class SolverApp:
    def __init__(self):
        ...
        for index, row in enumerate(solver_app_options["grid_config"]["rows"]):
            self.root.grid_rowconfigure(
                index, weight=row["weight"], minsize=row["minsize"]
            )

        for index, col in enumerate(solver_app_options["grid_config"]["cols"]):
            self.root.grid_columnconfigure(
                index, weight=col["weight"], minsize=col["minsize"]
            )
        ...
        for frame_name, frame_config in solver_app_options["frames"].items():
            frame = ctk.CTkFrame(
                ...
                fg_color=frame_config["bg"],
            )
            frame.grid(
                row=frame_config["grid_options"]["row"],
                rowspan=frame_config["grid_options"]["rowspan"],
        ...
        for canvas_name, canvas_config in solver_app_options["canvases"].items():
            ...
            canvas.pack(
                fill=canvas_config["pack_config"]["fill"],
                expand=canvas_config["pack_config"]["expand"],
            )
    ...
    def on_resize(self, event):
        ...
        self.controls_frame.configure(
            ...
            height=max(
                solver_app_options["frames"]["controls_frame"]["height"], window_height
            ),
        )

        self.maze_frame.configure(
            width=max(
                solver_app_options["frames"]["maze_frame"]["width"],
                int(
                    window_width
                    - solver_app_options["frames"]["controls_frame"]["width"]
                ),
            ),
            height=max(
                solver_app_options["frames"]["maze_frame"]["height"], window_height
            ),
        )
    ...
    def add_tabs(self, tabs_config):
        ...
        container = ctk.CTkFrame(
            self.controls_frame,
            fg_color=solver_app_options["frames"]["controls_frame"]["bg"],
            width=solver_app_options["frames"]["controls_frame"]["width"],
        )
        container.pack(
            expand=True, fill="y", pady=20
        )  # Add some padding at the top and bottom
        container.pack_propagate(False)

        # Create a CTkTabview inside the controls_frame
        self.tabview = ctk.CTkTabview(
            container, fg_color=solver_app_options["frames"]["controls_frame"]["bg"]
        )
    ...
    def add_slider(self, parent, config):
        ...
        slider = ctk.CTkSlider(
            parent,
            from_=config.get("from_", 0),
            to=config.get("to", 100),
            command=lambda value: self.handle_slider_change(
                config.get("command"), value
            ),
        )
    ...
    def add_dropdown(self, parent, config):
        ...
        dropdown = ctk.CTkOptionMenu(
            ...
            command=lambda value: self.handle_dropdown_change(
                config.get("command"), value
            ),
        )
    ...
    def handle_button_click(self, command):
        ...
        elif command == "select_maze_corners":
            self.manual_corner_input()
            ...
            for tab in self.tabview.winfo_children():
                if isinstance(tab, ctk.CTkFrame):
                    for widget in tab.winfo_children():
                        if (
                            isinstance(widget, ctk.CTkEntry)
                            and widget.cget("textvariable") == self.image_path_str
                        ):
                            widget.delete(0, "end")
    ...
    def process_maze_image(self):
        ...
        try:
            self.maze.graph, self.maze.rows, self.maze.cols, start_pos, end_pos = (
                self.maze.init_from_image_with_params(
                    image_path,
                    invert_binary=(self.invert_binary_str.get().lower() == "true"),
                    wall_threshold=int(self.wall_threshold_str.get())
                    if self.wall_threshold_str.get().isdigit()
                    else 127,
            ...
            messagebox.showinfo(
                "Success",
                f"Maze loaded successfully. Size: {self.maze.rows}x{self.maze.cols}",
            )
    ...
    def toggle_debug_mode(self):
        ...
        if current_value:
            messagebox.showinfo(
                "Debug Mode",
                "Debug mode enabled. Debug images will be saved to the 'debug' folder.",
            )
        else:
    ...
    def gen_display_algo_maze(self):
        logging.debug(
            f"Generating maze: {int(self.maze_rows_str.get()), int(self.maze_cols_str.get())}"
        )

        self.maze.gen_algo_maze(
            int(self.maze_rows_str.get()), int(self.maze_cols_str.get())
        )
        self.maze_canvas.delete("all")
    ...
    def manual_corner_input(self):
        ...
        new_image_path = self.maze.manual_corner_input(image_path)

        self.image_path_str.set(new_image_path)
